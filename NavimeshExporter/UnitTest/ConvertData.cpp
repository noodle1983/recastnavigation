...
void parseVerticeIndexes(rapidjson::Value& value, VerticeIndexes& vi){
    vi.clear();
    SizeType size = value.Size();
    vi.resize(size);
    for(SizeType i = 0; i < size; i++){
...
void logVerticeIndexes(VerticeIndexes& v){
    cout << "VerticeIndexes:";
...
int main(){
    auto doc = parseFile("./000_Walkable.markermesh");
    Document& data = *doc;
    ...
    VerticeIndexes vi;
    parseVerticeIndexes(data["simpleMesh"]["triangles"], vi);
    ...
    tileData.vertCount = vertices.size();
    ...
    tileData.triangleIndexCount = vi.size();
    ...
    if(ret){
		...
		{
			dtFree(outData);
			cout << "Could not create Detour navmesh" << endl;
			return false;
		}
		...
		{
			dtFree(outData);
			cout << "Could not init Detour navmesh" << endl;
			return false;
		}

        saveAll("a.bin", navMesh);
    }
